...
function BranchStockCard({ media, onAddToBasket }) {
  ...
  if (!media) {
    return <p>Loading media information...</p>;
  }
  const { user, basket, setBasket, branches, setBranches } =
    ...
    async function loadDataAndCheckStock() {
      try {
        const [branchData, stockData] = await Promise.all([
          branchFrontEndService.get("/readRecords", {
            BranchID: media.BranchID,
          }),
          mediaFrontEndService.get(
            "/readRecords",
            { Title: media.Title, Type: media.Type, BranchID: media.BranchID },
            false
          ),
        ]);
  ...
  return (
    <div
      className={
        isUserLocalBranch ? "content-panel-highlight-primary" : "content-panel"
      }
        ...
        <Col>
          {branch ? (
            <span>
              ...
              <br />
              {branch.City || "City"}, {branch.Postcode || "Postcode"}
              <br />
            </span>
          ) : (
